...
The pool is not used for every run. In a browser, threading a script that
allocates per deal is *slower* — wasm's allocator is one lock — so the engine
deals on the pool for a script that searches for double-dummy results and on one
thread otherwise. `docs/WASM.md` has the measurements.
