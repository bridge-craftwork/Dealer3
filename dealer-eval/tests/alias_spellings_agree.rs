...
/// `tricks` calls the double-dummy solver, which takes minutes per deal on the
/// legacy path — see issue #14. Its aliases are covered by the AST comparison
/// in dealer-parser instead.
fn uses_the_solver(name: &str) -> bool {
    name.contains("trick")
}
