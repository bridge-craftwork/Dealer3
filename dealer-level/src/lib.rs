...
pub fn write_leveled(
    ...
    let (generated, summary) =
        render_leveled(source, plans, lambda, acceptance, base_rate, measured, seed)?;
...
pub struct Leveled {
    ...
    pub script: String,
    /// The same numbers as a block of text, for a terminal.
    pub summary: String,
...
pub fn level_from(
    ...
    let (script, summary) = render_leveled(
        source,
        &plans,
        lambda,
        acceptance,
...
/// only `run`, since that is the one part they genuinely differ in — the command
/// line deals across a thread pool with CSV and a timeout, the browser in a
/// single loop it must not block. `final_pass` says which of the two is being
/// asked for, so the measuring pass can skip keeping and ordering deals nobody
/// will look at.
///
...
pub fn level_and_run<T>(
    source: &str,
    opts: &LevelOptions,
    mut run: impl FnMut(&str, usize, bool) -> Result<(T, Measurement), String>,
) -> Result<(T, LevelReport), String> {
    let prepared = insert_leveling_block(source)?;
    check_leveling_source(&prepared)?;

    let (_, measured) = run(&prepared, opts.measure_produce, false)?;
    ...
    let (outcome, delivered) = run(&leveled.script, usize::MAX, true)?;
