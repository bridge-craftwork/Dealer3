...
pub fn insert_leveling_block(source: &str) -> Result<String, String> {
    ...
    let gate = if VERDICTS.iter().any(|name| source.contains(name)) {
        String::new()
    } else {
        format!(" and {verdict}")
    };
...
/// The names a scenario may use for the verdict the condition gates on.
///
/// Two of them because scenarios in the wild were written both ways before the
/// generator existed, and renaming someone's working script is not worth it.
/// The first is what a block written from scratch uses.
pub const VERDICTS: [&str; 2] = ["levelTheDeal", "keepTheDeal"];

/// The placeholder verdict a stock scenario carries, standing in for the
/// generated arithmetic so the unlevelled file still runs.
pub const NO_LEVELING: &str = "noLeveling";
