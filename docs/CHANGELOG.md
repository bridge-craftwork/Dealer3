...
### Fixed
- **The web page's PBS tab showed a leveled scenario's stock script instead of
  the one PBS publishes** (#132). Every script was fetched from `dlr/`, but PBS
  publishes the 69 scenarios it levels from `dlr-leveled/`, generated by
  `dealer --write-leveled`, and its manifest says which file each scenario is.
  A `_Leveled` button and the plain one beside it are two scenarios; the stock
  file behind the leveled one still says `levelTheDeal = noLeveling`, so the
  page showed — and, with Auto-level off, ran — an unlevelled script. The page
  now fetches the file the manifest names, and most of those 69 are not called
  `_Leveled` (1N, Stayman, NT_Ladder). A generated script levels itself, so
  Auto-level stays off for it: the engine refuses to measure an
  already-levelled mix, which would compute keeps of about 1 and level nothing.
- **Auto-level could stay off for a scenario it should have levelled.** It ticks
  itself for a script that names hand types unless somebody has chosen
  otherwise, and the page mistook its own changes for that choice: opening any
  scenario without hand types switched it off and counted as a choice, and so
  did every return visit, because the session always stores the setting. Only a
  click on the box, or a document that states it — a demo, a History version, a
  link — now counts, and opening a PBS scenario resets it to that scenario's
  default.
